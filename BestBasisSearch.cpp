...
vector<Operator64> BestBasisSearch_FixedRepresentation(vector<pair<uint64_t, unsigned int>> Nvect, unsigned int n, unsigned int N, unsigned int k_max, unsigned int B_it, bool bool_print = false)
{
  ...
  set<Operator64> OpSet = LowerBound_Op1_bias_vect(Nvect, n, N, &Bias_LowerBound, bool_print); 
...
*/
  ...
  for (unsigned int k = 2; k <= k_max; k++)
  {
      ...
      Add_Op_kbits_MostBiased_rank64_vect(OpSet, Nvect, n, N, k, LB.Bias, bool_print);
...
//    cout << endl << "*******************************************************************************************" << endl;
      ...
      BestBasis.clear();
      BestBasis = Best_Basis_inOpSet_F2_Mat(OpSet, n, &LB, 1000); // LB will be over-written with the updated values
